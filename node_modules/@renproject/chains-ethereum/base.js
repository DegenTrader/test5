...
class EthereumBaseChain {
    constructor({ network, provider, signer, config, }) {
        ...
        this.transactionConfidence = async (transaction) => {
            ...
            if (receipt.status === 0) {
                throw utils_1.ErrorWithCode.updateError(new Error(`${String(transaction.chain)} transaction failed: ${transaction.txHash}`), utils_1.RenJSError.CHAIN_TRANSACTION_REVERTED);
            }
        ...
        this.populateChainTransaction = (partialTx) => {
            return (0, utils_1.populateChainTransaction)({
                partialTx,
                chain: this.chain,
        ...
        this.withProvider(provider || (0, generic_1.resolveRpcEndpoints)(this.network.config.rpcUrls)[0]);
        if (signer) {
            this.withSigner(signer);
        }
    }
