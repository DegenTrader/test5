 ...
 * PRC url variables.
 */
const getEVMProvider = (networkConfig, key, variables) => {
    const urls = (0, exports.resolveRpcEndpoints)(networkConfig.config.rpcUrls, variables);
    const provider = new ethers_1.ethers.providers.JsonRpcProvider(urls[0]);
    const signer = key.privateKey !== undefined
        ? new ethers_1.Wallet(key.privateKey)
        : ethers_1.Wallet.fromMnemonic(key.mnemonic, `m/44'/60'/0'/0/${String(key.index || 0)}`).connect(provider);
    return {
        provider,
        signer,
    };
